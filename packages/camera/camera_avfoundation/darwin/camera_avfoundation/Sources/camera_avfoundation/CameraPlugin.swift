...
public final class CameraPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let instance = CameraPlugin(
      registry: registry,
      messenger: messenger,
      globalAPI: CameraGlobalEventApi(binaryMessenger: messenger),
      ...
      deviceFactory: { name in
        ...
        DefaultCaptureDevice(avDevice: AVCaptureDevice(uniqueID: name)!)
      },
      captureSessionFactory: { DefaultCaptureSession(avSession: AVCaptureSession()) },
    ...
    CameraApiSetup.setUp(binaryMessenger: messenger, api: instance)
  }
  ...
  private static func pigeonErrorFromNSError(_ error: NSError) -> PigeonError {
    ...
    func orientationChanged(_ notification: Notification) {
      ...
      let systemOrientation = device.orientation

      if systemOrientation == .faceUp || systemOrientation == .faceDown {
        // Do not change when oriented flat.
        return
      }

      let orientation = getPigeonDeviceOrientation(for: systemOrientation)
  ...
  func sendDeviceOrientation(_ orientation: PlatformDeviceOrientation) {
...
extension CameraPlugin: CameraApi {
  ...
  func getAvailableCameras(
    ...
    captureSessionQueue.async { [weak self] in
      ...
      let discoveryDevices = strongSelf.supportedDiscoveryDeviceTypes()
  ...
  private func supportedDiscoveryDeviceTypes() -> [AVCaptureDevice.DeviceType] {
    #if os(iOS)
      return [
  ...
  private func sessionQueueCreateCamera(
    ...
    let camConfiguration = CameraConfiguration(
      ...
      audioCaptureDeviceFactory: {
        DefaultCaptureDevice(avDevice: AVCaptureDevice.default(for: .audio)!)
      },
  ...
  private func sessionQueueInitializeCamera(
    ...
    sendDeviceOrientation(camera.deviceOrientation)
    camera.start()
    completion(.success(()))
  }
